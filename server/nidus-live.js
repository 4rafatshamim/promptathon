(() => {
  ...
  function makeButton(label, action) {
    ...
    button.type = "button";
    button.className = "rounded-lg border border-bordercream bg-softcream px-3 py-2 text-[10px] font-bold hover:border-studio";
  ...
  async function uploadSelected(fileInput) {
    ...
    for (const file of selected) {
      ...
      try {
        const result = await readJson(await fetch(api + "/files", { method: "POST", body: form }));
  ...
  async function verifyAndRepair(file) {
    const status = {};
    const messages = [];
    for (const node of nodes) {
      try {
